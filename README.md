...
It requires no sign-in, external libraries, or build process. Open the file in a modern browser, or publish the `artifact/` folder using a static hosting service such as GitHub Pages.
...
The `artifact/` directory can be deployed using any static hosting service.

For GitHub Pages, configure Pages to serve the repository and open the `/artifact/` path in the published site.

After deployment, add the public artifact URL and repository URL to the competition submission form and, if desired, to this README.
